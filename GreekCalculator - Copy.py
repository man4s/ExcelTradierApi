...
def optionChainGreeks(spot,
    ...
    if (filePath != ''):
        putData = retData[retData['Option Type'] == 'p']
        callData = retData[retData['Option Type'] == 'c']
        putData.to_csv(filePath + symbol + '_' + maturity + 'put.csv', index=False)
        callData.to_csv(filePath + symbol + '_' + maturity + 'put.csv', index=False)
...
def optionChainScenarioGreeks(spot,
    ...
    retData = pd.DataFrame({ str(1- spotDevalue) + ' Spot Option Price' : sSprices,
                             '(' + str(T*365) + ' + ' + str(timeIncrease*365) + ' days)  Option Price' : sTprices,
